...
First, you will need to create a new file at `pages/api/faust/[[...route]].ts` with the following content:
...
Then, you will need to delete the `pages/api/auth/wpe-headless.ts` file if it exists.

Finally, replace your current `faust.config.js` file with the following:
