...
export function initialize(client: ApolloClient<NormalizedCacheObject>) {
    return {
        posts() {
            return posts(client);
        },
        post(uid: string, idType?: PostIdType) {
            return post(client, uid, idType);
        },
        revision(revisionId: string) {
            return revision(client, revisionId);
        },
    };
}
