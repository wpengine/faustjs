...
The example project contains a site demostrating the use of Faust features like WordPress template hierarchy and the plugin system.
...
`components`: This folder contains the varius components that all pages and templates use.
They are highly re-usable and for purely presentational purposes.

`constants`: This folder contains a few harcoded constants that are used in queries.
...
For each type of page that correspond to the WordPress template hierachy, there are different layouts and queries.
