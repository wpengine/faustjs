...
The plugin extends schema with new graphql types and fields.
...
Expanding the `PostBlock` inteface docs, shows that the type only implements the two blocks we declared in the `allowed_block_types_all` filter:
