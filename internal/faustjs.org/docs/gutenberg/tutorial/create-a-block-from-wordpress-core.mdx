...
### 4.2 Create the block file inside the `wp-blocks` folder
...
### 4.3 Create an `index.js` file inside the `wp-blocks` folder that imports and then exports the `CoreCode` block. You will use this same file for other blocks as well.
