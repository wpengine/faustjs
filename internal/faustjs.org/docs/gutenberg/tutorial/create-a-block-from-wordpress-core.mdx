...
### 3.1. Within the GraphiQL IDE, use the Docs to find the `CoreCode` Block to display its type and properties. If you chose a different Block, then search for that type instead.
...
### 3.2. You can use the following query in the GraphiQL IDE to inspect the Block data. Replace `/posts/testing` with the path to your post.
...
### 4.3 Create an `index.js` file inside the `wp-blocks` folder that imports and then exports the `CoreCode` block. You will use this same file to export other blocks as well.
...
You want to create a fragment that describes the request block fields and attributes. Include all relevant fields for your implementation. The `CoreCode` file so far should look like this:

```jsx title="wp-blocks/CoreCode.js" {4,11-26}
// import react
import React from 'react';
...
You want to include the fragment in your Faust templates `query` string and reference the block fragment `key` so that when the page resolves, the payload includes the data from that block.
...
Now you are ready to design the block based on the original block's look and feel. The last part is to implement the Block by using the block attributes and the view in the editor to construct the final HTML markup.
