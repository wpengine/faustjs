...
Notice that in this template example we first `import blocks from '../wp-blocks'`. Then inside the templates gql `Component.query`, we include the fragment entry with `${blocks.CoreCode.fragments.entry}`. Then we use the key `${blocks.CoreCode.fragments.key}` inside the `GetPost`. In this case it is inside the `editorBlocks` which is inside the `post`.
