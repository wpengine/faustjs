...
## 2. Download a third-party plugin that provides Guttenberg Blocks
...
Feel free to tweek the block in the editor and inspect the changes reflected in the website.
...
is dependent on the inclusion of this plugin. You need to make sure you bundle them together otherwise the queries you do in the Decoupled Application will fail.
:::
...
* **Just include them in your code:** This is not recommended as React does not play well with plain JavaScript and jQuery so you can expect lots of problems.
...
* **Just use a third party equivalent React Component:** A simpler alternative is to find a compatible React Package and use that instead of trying to replicate the blocks interactivity. Sometimes this is a viable strategy as it frees the developer from implementing the functionality from scratch.
