...
A few important things to consider here:
 ...
 - First we assign the same className `my-block-code` for the block so that we can apply the same styles as the Editor.
...
...
@import './blocks/MyCoolBlock' // import block styles here
```
