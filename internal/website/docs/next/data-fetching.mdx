...
import { client, getNextStaticProps } from '@wpengine/headless-next';
import { GetStaticPropsContext } from 'next';
import {PostIdType} from 'src/client'
...
import { client, getNextStaticProps } from '@wpengine/headless-next';
import { GetStaticPropsContext } from 'next';
import {PageIdType} from 'src/client'
...
A hook to retrieve multiple pages.
...
A hook to retrieve the page/post from a preview URL. Can only be used in client side requests.
...
If you want to make a custom query, use the `useQuery` hook. This hook will give you access to your entire WordPress schema.
