---
slug: /next/reference/helpers/getNextServerSideProps
...
The `getNextServerSideProps` function accepts two arguments: the server side props `context`, and an object of type `GetNextServerSidePropsConfig` with a `Page` key.
...
The secondary argument object of the `getNextServerSideProps` is of type `GetNextServerSidePropsConfig` and accepts the following parameters:
...
* `props`: The `props` object is any other key value pairs of properties that can be forwarded to the `Page` component.
