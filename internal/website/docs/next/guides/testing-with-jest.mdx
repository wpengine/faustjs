...
As a generic example, let's create a test case that check for the rendered navigation items that we have hardcoded, and have received from GraphQL via GQty. Create a file called `src/components/Header.test.tsx`.
