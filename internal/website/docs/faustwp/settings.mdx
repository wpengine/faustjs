...
For example if your WP `home_url` is `http://example.com` and your `Front-end site URL` is `http://headless.com`
then if you visit any page on your `home_url` will be redirected to the headless `Front-end site URL` passing any query params as well:
...
For example, if your WP `home_url` is `http://example.com` and your `Front-end site URL` is `http://headless.com`
then any images served from headless `Front-end site URL` will be rewritten to point to the `home_url`:
