...
description: Deploying your Faust.js app
---
...
Atlas is the most effortless way to deploy a Faust.js app. Atlas works by connecting to your Faust.js GitHub repo, and upon changes, Atlas will automatically build and deploy it. Some of the benefits you get out of the box are:
...
- Automatic setup of Faust environment variables
...
Faust.js can also be self hosted by using any hosting provider that supports Node.js.

To self host your Faust.js app, you will need to first build the app, then start it. This is done by having the `next build` and `next start` scripts specified in your `package.json` file:
...
After adding the following scripts to your `package.json`, run `npm run build`, and then `npm run start` from the Node.js server to start your app.
