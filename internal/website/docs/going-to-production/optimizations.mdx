...
You should ensure that these data selections are outside of conditionals (`if`, `switch`, etc. statements), as GQty will not be able to appropriate fetch this data sever side. Thus, this data will be cascaded to the client, and fetched there. This is appropriate when you need to fetch data specifically on the client side, but often times you wan't this data to be fetched server side so it is cached.
...
Next.js has a "Going to Production" guide that will go into further detail on the optimizations you can make for your Faust.js app build on top of Next.js:
