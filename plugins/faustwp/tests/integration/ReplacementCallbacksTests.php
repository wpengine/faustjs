...
class ReplacementCallbacksTests extends \WP_UnitTestCase {
	...
	public function test_post_preview_link_adds_preview_true_query_param() {
		...
		$this->assertSame( 'http://moo/?p=' . $this->post_id . '&preview=true&previewPathname=' . urlencode( wp_make_link_relative( get_permalink( $this->post_id ) ) ) . 'typeName=Post', $link );
	}
	...
	public function test_custom_post_type_post_preview_link_returns_filtered_link_when_content_replacement_is_enabled()
	{
		...
		$this->assertSame( 'http://moo/?document=' . $post_id . '&preview=true&' . urlencode( wp_make_link_relative( get_permalink( $post_id ) ) ) . '&p=' . $post_id . '&typeName=Document', get_preview_post_link( $post_id ) );
		faustwp_update_setting( 'frontend_uri', null );
	...
	public function test_post_preview_link_filters_link_for_posts_not_registered_with_wpgraphql() {
		...
		$this->assertSame( 'http://moo/?notgraphql=' . $post_id . '&preview=true&' . urlencode( wp_make_link_relative( get_permalink( $this->post_id ) ) ) . '&p=' . $post_id, get_preview_post_link( $post_id ) );
	}
